...
def sinc(band, t_right):
    y_right = torch.sin(2 * math.pi * band * t_right) / (
...
class sinc_conv(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.N_filt):
            low_pass1 = 2 * filt_beg_freq[i].float() * sinc(
                filt_beg_freq[i].float() * self.freq_scale, t_right
            )
            low_pass2 = 2 * filt_end_freq[i].float() * sinc(
                filt_end_freq[i].float() * self.freq_scale, t_right
            )
